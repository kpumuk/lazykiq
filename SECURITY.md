...
Security fixes are provided for the latest released version of `lazykiq`.
...
`lazykiq` is a terminal UI for Sidekiq monitoring. Some behavior may depend on Redis, Sidekiq, terminal emulators, or local environment configuration. If a report depends on a specific setup, include those details.
